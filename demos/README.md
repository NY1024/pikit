...
**攻击构造** (the injected payload and the exact tainted artifact delivered)
...
| `data_sample` | `webpage` `email` `document` `code` `skill` | per-agent (see below) | which clean built-in sample to taint |
